...
-   子节点Female的Gini指数 =（0.2）_（0.2）+（0.8）_（0.8）= 0.68
-   子节点Male的Gini指数 =（0.65）_（0.65）+（0.35）_（0.35）= 0.55
-   计算基于性别分裂的加权Gini指数=（10/30）_ 0.68 +（20/30）_ 0.55 = 0.59

#### 相似地,基于年级的分裂：

-   子节点类IX的Gini指数 =（0.43）_（0.43）+（0.57）_（0.57）= 0.51
-   子节点类X的Gini指数=（0.56）_（0.56）+（0.44）_（0.44）= 0.51
-   计算基于年级分裂的加权Gini指数=（14/30）_ 0.51 +（16/30）_ 0.51 = 0.51
...
$$ s^2={\\frac1n}\\sum\_{i=1}^n(x_i-\\overline{x})^2 $$
